...
public static class ProcessControlService
{
    ...
    /// <summary>
    /// Multi-core preference. Two mechanisms combined, WITHOUT touching the affinity mask:
    ///   1. Process default CPU Sets (SetProcessDefaultCpuSets with real CPU-set IDs) — the
    ///      kernel's actual soft-affinity: the scheduler PLACES threads on these cores.
    ///      (The old code passed a raw bitmask to NtSetInformationProcess(0x42), which is
    ///      the wrong argument type — the call succeeded but pinned nothing. That was the
    ///      real reason "优先核心" never held.)
    ///   2. Per-thread ideal processor round-robin as an extra placement hint.
    /// Re-applied periodically by the watchdog so threads created later are covered too.
    /// </summary>
    public static bool ApplyPreferredCores(int pid, ulong preferredMask)
    {
        var cores = CoresOf(preferredMask);
        if (cores.Count == 0) return false;

        bool setsOk = SetDefaultCpuSets(pid, preferredMask);
    ...
    private static uint[]? CpuSetIdsFor(ulong mask)
    {
        try
        {
            ...
            for (int i = 0; i < 64; i++)
                if ((mask & (1UL << i)) != 0 && _cpuSetIdByCore[i] != 0)
