...
public class EnforcementService : IEnforcementService
{
    ...
    public bool Apply(int pid, RuleEntry rule, CpuTopology topology)
    {
        ...
        // Scheduling pool only restricts affinity for static/d2 binding modes.
        // dynamic/d3 use IdealProcessor hints only — all cores stay usable.
        string pm = rule.Action.GetPreferMode();
        if (pm is "static" or "d2")
        {
        ...
        if (applied)
        {
            ...
            if (rule.Action.GpuPriority is int gpuPrio && gpuPrio >= 0 && gpuPrio <= 5)
                ProcOps.ProcessControlService.SetGpuPriority(pid, gpuPrio);
        }
