...
public class CpuTopologyService : ICpuTopologyService
{
    ...
    private static CpuTopology DetectInternal()
    {
        ...
        // Step 2b: Build per-physical-core sibling masks for UI auto-pairing
        ulong[] smtSiblingMask = BuildSmtSiblingMasks(totalLogical, smtEnabled);
    ...
    /// <summary>
    /// Builds a per-logical-processor array of physical-core sibling masks.
    /// SmtSiblingMask[i] = all logical processors in the same physical core as i.
    /// Uses GetLogicalProcessorInformation RelationProcessorCore entries.
    /// </summary>
    private static ulong[] BuildSmtSiblingMasks(int totalLogical, bool smtEnabled)
    {
        var result = new ulong[64];
        if (!smtEnabled)
        {
            for (int i = 0; i < Math.Min(totalLogical, 64); i++)
                result[i] = 1UL << i;
            return result;
        }
        ...
        try
        {
            ...
            while (offset + structSize <= (int)bufferSize)
            {
                ...
                if (info.Relationship == LOGICAL_PROCESSOR_RELATIONSHIP.RelationProcessorCore)
                {
                    ulong coreMask = (ulong)info.ProcessorMask;
                    for (int i = 0; i < Math.Min(totalLogical, 64); i++)
                        if ((coreMask & (1UL << i)) != 0)
                            result[i] = coreMask;
                }
